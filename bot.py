...
def _loaded_model_identifiers(config: Config) -> list[str]:
    output = _run_lms(config, ["ps"], timeout=90)
...
def _check_loaded_model_guard(config: Config, previous_identifier: str, target_identifier: str) -> None:
    loaded = _loaded_model_identifiers(config)
    blockers = [
        identifier
        for identifier in loaded
        if identifier not in {previous_identifier, target_identifier}
    ]
    if blockers:
        raise RuntimeError(
            "Refusing to load another model while these non-active models are already loaded: "
            + ", ".join(blockers)
            + ". Use `!lm loaded` to inspect LM Studio first."
        )
...
def _check_load_guard(config: Config, previous_identifier: str, target_identifier: str) -> None:
    _check_loaded_model_guard(config, previous_identifier, target_identifier)
...
def _command_name(prompt: str) -> str:
    ...
    if not lower:
        return "empty"
    first = lower.split()[0]
    if first in {"health", "status", "current", "models", "list", "loaded", "ps", "use", "load"}:
        return first
    return "chat"
            ...
            async with self.request_semaphore:
                async with message.channel.typing():
                    try:
                        ...
                        elif command in {"models", "list", "loaded", "ps", "status", "current", "use", "load"}:
